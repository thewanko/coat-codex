...
describe("ExportActionBar — PC幅（従来のピル群）", () => {
  ...
  test("recipeがnullのとき印刷・SNSに投稿・note MD・JSON・素MDの5ボタンをすべてdisabledで配置する", () => {
    renderBar(null);

    const labels = ["印刷", "SNSに投稿", "note MD", "JSON", "素MD"];
  ...
  test("recipeがある場合、印刷・SNSに投稿は有効化される（T40結線・2026-07-04 FB-A統合）", () => {
    renderBar(makeRecipe());

    for (const label of ["印刷", "SNSに投稿"]) {
  ...
  test("ShareDialog内のBlueskyタブへ切替できる", async () => {
    vi.stubGlobal("navigator", { canShare: () => true, share: vi.fn() });
    composeShareImagesMock.mockResolvedValue([]);
...
describe("ExportActionBar — mobile幅（出力・共有ボタン→ボトムシート）", () => {
  ...
  test("recipeがある場合、シート内の印刷・SNSに投稿は有効化される（T40結線・2026-07-04 FB-A統合）", () => {
    renderBar(makeRecipe());
    fireEvent.click(screen.getByRole("button", { name: "出力・共有" }));
  ...
  test("シート内の「SNSに投稿」ボタン押下でShareDialogがwholeコンテキストで開く（2026-07-04 FB-A統合）", async () => {
    vi.stubGlobal("navigator", { canShare: () => true, share: vi.fn() });
    composeShareImagesMock.mockResolvedValue([]);
    ...
    const dialog = screen.getByRole("dialog");
    fireEvent.click(within(dialog).getByRole("button", { name: "SNSに投稿" }));
  ...
  // 2026-07-04 FB-G: .mobileRootをposition: fixed→stickyへ変更した際、fixed全幅化対策
  // だったpointer-events: noneと.overlayRoot打ち消しラッパーを撤去した（sticky化で
  // 通常フローの一員になり全幅を専有しなくなったため不要）。MarkdownCopyFallbackDialogが
  // 引き続き.mobileRootの子孫としてレンダーされ、独立して開閉できることを確認する。
  test("MarkdownCopyFallbackDialogは.mobileRootの子孫としてレンダーされ、独立して開閉できる（sticky化後もリフトアップ維持）", async () => {
    Object.assign(navigator, { clipboard: undefined });
    ...
    expect(mobileRoot.contains(fallbackBackdrop)).toBe(true);
  });
  ...
  // 既知バグ修正（2026-07-04 FB-G申し送り）: MobileExportRootがExportSheetへ渡す
  // onCloseがインライン関数（毎レンダー新規参照）だと、ExportSheetのフォーカスeffect
  // （deps: [open, onClose]）がonCloseの参照変化のたびに再実行され、シート表示中に
  // 親の無関係なstate変化（noteMdCopiedのタイマー切替等）が起きるたびに✕ボタンへ
  // フォーカスが奪われていた。onClose安定化（useCallback）後は、シート内操作で
  // ボタンラベルへフォーカスした状態のまま親stateが変化しても、フォーカスが✕へ
  // 移らないことを確認する。
  test("シート表示中に親の無関係なstate変化（note MDコピー成功のラベル切替）が起きてもフォーカスは✕ボタンへ移らない（FB-G onClose安定化）", async () => {
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.assign(navigator, { clipboard: { writeText } });
    ...
    const dialog = screen.getByRole("dialog");
    const noteMdButton = within(dialog).getByRole("button", {
      name: "note MD",
    });
    noteMdButton.focus();
    expect(noteMdButton).toHaveFocus();

    fireEvent.click(noteMdButton);

    // noteMdCopied=trueへの状態変化でMobileExportRoot・ExportSheetが再レンダーされる
    // （ラベルが「コピーしました ✓」に切り替わる＝親stateが変化した証跡）。
    await waitFor(() => {
      expect(
    ...
    // onCloseが安定していればExportSheetのフォーカスeffectは再実行されず、
    // フォーカスは✕ボタン（閉じる）へ奪われない。
    expect(screen.getByRole("button", { name: "閉じる" })).not.toHaveFocus();
  });
});
