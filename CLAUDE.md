...
- アニメーション付きオーバーレイUIは座標検証前に `el.getAnimations().forEach(a => a.finish())`（非表示タブはCSSアニメーションが currentTime=0 で停止する）。**さらに背景タブはviewportが0×0のままレイアウトされず`elementFromPoint`が全対象でnull（偽陰性）になることがある**＝座標検証の前に`innerWidth > 0`を確認し、0ならpreview_screenshot/preview_resizeで描画を強制してから検証する（スクショは正常描画でもevalの座標系が死んでいる、という乖離が起きる） (lessons.md昇格 2026-07-08: 非表示タブのレンダリング停止2回目=viewport相)
