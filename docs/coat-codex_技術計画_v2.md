...
- 工程エディタ（`ToolSelect`）でユーザーがツールを選択・新規追加した際は、`doc.tools`に`toolNameKey`同名が**まだない**場合に限り、ライブラリの`UserToolRecord`から`{ id: "tool_<uuid>"（新規採番）, name, note }`を`doc.tools`へ**コピー**する（参照ではない）。これにより、**ライブラリからツールを削除しても既存レシピの`doc.tools`・工程（`toolIds`）には一切影響しない**。同名が既に`doc.tools`にある場合はコピーせず、既存の`Tool`を当該工程へチェックするのみとする（v2.8改訂・後述「サジェスト全候補化」参照）
...
**サジェスト全候補化（v2.8新設: 2026-07-14ユーザーFB裁定）**:

- `ToolSelect`のサジェスト候補は、従来の「`doc.tools`に`toolNameKey`同名が存在しないライブラリツール」に絞る**dedupe仕様を廃止**し、ライブラリの**全ツール**を常に候補表示する（draft部分一致・タグ絞り込みは従来どおり適用）。旧仕様のまま一括移行（上記）を行うと、旧レシピの`doc.tools`がライブラリ全ツールと同名化し、サジェスト候補が常に0件へ縮退して見える不具合があったため
- 候補クリック時の挙動: `doc.tools`に`toolNameKey`同名が**ある**場合は新規`Tool`をコピーせず、既存ツールの`id`を当該工程の`toolIds`へチェックする（既にチェック済みなら変化なし＝トグルオフはしない）。同名が**ない**場合は従来どおりライブラリの`{ name, note }`を`doc.tools`へコピーし当該工程へ即チェックする
- 視覚区別: 同名が`doc.tools`にあり当該工程で既にチェック済みの候補ボタンは`aria-pressed="true"`で強調表示する
...
react-router v7（declarative mode、`BrowserRouter`。SPAフォールバックは§5.2の方針＝`_redirects`なし・`404.html`非配置）。**全10ルート**で要件の全画面＋使い方・Q&A＋ツールライブラリ管理＋更新履歴を賄う。
...
>
> **v2.9改訂経緯（2026-07-14ユーザーFB裁定）**: 機能更新履歴をアプリ内ページとして持つ要望を受け、静的ページ `/updates`（UpdatesPage）を新ルートとして追加する。**あわせて、`/help`（HelpPage・使い方・Q&A）が2026-07-05に実装済みでありながら本節に一度も記載されていなかったドキュメントドリフトを本改訂で是正し、遡及文書化する**（実装自体はv2.9より前の既存機能。本節の記述漏れのみを修正するものであり、機能追加ではない）。ルート数はこれら2件の反映により8から**10**へ改訂する。
...
| `/help` | HelpPage | **使い方・Q&A**（6ステップの使い方案内＋データ／対応環境／共有・出力のQ&A、1ページ統合）。**v2.9で遡及文書化**: 2026-07-05に実装済みだったが本節に未記載だったドキュメントドリフトであり、機能追加ではない。`AppFooter`から常設導線 |
| `/tools` | ToolsPage | **v2.6追加**: ツールライブラリ管理（一覧・追加・タグ付け・削除・専用エクスポート/インポート。§2.8/§3.3）。`AppFooter`から常設導線＋**v2.9追加: `HomePage`アクション行からも直接遷移可（§3.3）** |
| `/updates` | UpdatesPage | **v2.9追加**: 機能更新履歴（静的コンテンツ・i18n。HelpPageと同型のBackLink→hero→エントリ列構成）。`AppFooter`から常設導線（§3.3・§4.2 T67） |
...
| 画面 | 主要コンポーネント | 責務 |
|---|---|---|
| **AppShell**（全画面共通） | `LanguageSwitcher` / `AppFooter` / `ToastHost` | ja/en切替（localStorage永続化）／**`AppFooter`は`/terms`・`/help`・`/tools`・`/updates`の4リンク常設（菱区切り。v2.9で`/updates`を追加し4リンク化。§4.2 T67）**／保存・エラー通知 |
...
| | `ToolLibraryButton`（**v2.9追加・2026-07-14ユーザーFB裁定・§4.2 T65**） | アクション行の3つ目のボタンとして`NewRecipeButton`・`ImportJsonButton`と同格のピルで並置（モバイル折返し可）。押下で`/tools`へ遷移する純粋なナビゲーション（`storage.persist()`要求なし）。**ツールライブラリはレシピ横断のメイン機能であり、`AppFooter`常設導線に加えHomeの目立つ位置にも導線を置く**（M13マージ後ユーザーFB裁定）。**空状態`EmptyState`（homeバリアント）のCTAはレシピ作成が主目的のため、従来どおり新規作成・インポートの2つのままとし本ボタンは追加しない** |
...
| | ~~PaletteEditor~~ / ~~ToolListEditor~~ / `ToolLibraryHint` | **v2.3: 使用カラーの先行登録は廃止**（色は工程のPaintPickerからのみ追加。**参照0のpalette色は保存時に自動GC** — custom色のチップ写真Blobも`deletePhoto`で回収。§2.6の削除ガードはツール側のみ存続）。~~`ToolListEditor`（使用ツールの先行登録）は維持 — 各行に使用数バッジ・使用中削除不可（スキーマ§2.6）。v2.6追記: 新規ツール名の追加時、同名（`toolNameKey`一致）がツールライブラリ（§2.8）に無ければ`registerUserTool`で自動登録（fire-and-forget・失敗はconsole.warnのみ）~~ — **v2.3〜v2.8時点の記録として据え置く（歴史記述）**。**v2.9改訂（2026-07-14ユーザーFB裁定・§4.2 T66）: `ToolListEditor`はSetupから廃止する**。Setupにはツール登録UIを置かず、代わりに`ToolLibraryHint`（1行の誘導文言＋`/tools`へのLinkと工程エディタ〔`ToolSelect`〕への案内。「ツールはツールライブラリまたは工程編集画面から登録できます」相当）のみを表示する。ツール登録経路は`/tools`（ToolsPage・§2.8）と工程エディタ（`ToolSelect`・その場追加）の2経路に一本化される |
...
| **UpdatesPage**（**v2.9新設・§4.2 T67**） | `BackLink`／hero（overline＋タイトル＋gloss）／エントリ列（日付mono＋見出し＋本文の`section`を新しい順に列挙） | 機能更新履歴の静的コンテンツ（i18n）。**構造イディオムはHelpPage/TermsPageに準拠**（フラットルート・オーバーライン＋明朝見出し）。i18n `updates.*`は文字列リテラルキーのみ使用（`i18n.test.ts`の静的キー抽出のため、エントリ配列のmap化はしない。HelpPageの規約を踏襲）。初期コンテンツは初期リリース〜M13の主要機能をユーザー向け文言で日付付きにまとめる |
...
##### M13: ToolSelectサジェスト全候補化＋ToolsPageモバイル行修正（v2.8新設: 2026-07-14ユーザーFB裁定。§2.8/§3.3反映）

> M12の一括移行後、旧レシピはライブラリ全ツールと`doc.tools`が同名化し、dedupe仕様のままではサジェスト候補が常に0件へ縮退して見える不具合のFB対応（T62）と、ToolsPageのモバイル行レイアウトの修正（T63）の2点セット。

| # | タスク | 成果物ファイル | 依存 | テスト |
|---|---|---|---|---|
| 62 | **ToolSelectサジェスト全候補化**: 「doc.toolsに同名がないライブラリツール」に絞るdedupeを廃止し、ライブラリ全ツールを常に候補表示する。候補クリックは`doc.tools`に`toolNameKey`同名があれば新規コピーせず既存ツールのidを当該工程のtoolIdsへチェックのみ行い（doc.toolsは不変）、同名がなければ従来どおりdoc.toolsへコピー＋即チェックする。同名かつ当該工程でチェック済みの候補は`aria-pressed="true"`で視覚区別する | `src/components/part-editor/ToolSelect.tsx`更新 / 同CSS更新 / テスト（既存dedupe前提1件を新仕様へ改変＋新規） | 56 | ✅（全ツール候補表示・同名クリックで既存idチェック＋doc.tools不変・同名チェック済み候補のaria-pressed・同名なし候補は従来どおりコピー・NFC/NFD同名の既存扱い） |
| 63 | ToolsPageモバイル行レイアウトの再構成（並列impl実装） | `src/routes/ToolsPage.tsx`更新 / `docs/design/coat-codex_デザイン仕様書.md`更新 | — | ✅ |

**M13完了条件**: T62〜T63がreview PASS＋既存ToolSelect/ToolsPageテスト（新仕様への改変分を除き）無改変pass＋出口実機で「旧レシピ（doc.toolsがライブラリと同名）を開いてもサジェスト候補が表示される・同名候補クリックでdoc.tools件数が増えない」

##### M14: Home導線＋Setupツール登録廃止＋更新履歴ページ（v2.9新設: 2026-07-14ユーザーFB裁定。§3.1/§3.3反映）

> M13マージ後のユーザーFB3点セット（①ツールライブラリはメイン機能でありフッターだけでなくHomeアクション行にも導線を置く ②Setupのツール先行登録UIを廃止しツールライブラリ/工程エディタへの誘導文言に置換する ③アプリ内に機能更新履歴ページ`/updates`を新設する）に対応する。各タスクの完了条件は「`npm run build`・`npm run lint`・ルート`npm test`が全てexit 0（新規テスト含む）」を共通前提とし、個別の実機検証項目を付記する。

| # | タスク | 成果物ファイル | 依存 | テスト |
|---|---|---|---|---|
| 64 | **仕様書v2.9改訂＋デザイン仕様書改訂**（本タスク・docs only）: 改訂履歴にv2.9行・§3.1に`/updates`追加＋実装済み未記載だった`/help`を遡及文書化（総数表記は10ルートへ是正）・§3.3のAppShell/HomePage/Setup/UpdatesPage記述更新・本§4.2にM14追加。あわせて`docs/design/coat-codex_デザイン仕様書.md`にHomeアクション行3ボタン化・`AppFooter`4リンク化・Setup誘導文言・UpdatesPageの意匠節を追加 | `docs/coat-codex_技術計画_v2.md` / `docs/design/coat-codex_デザイン仕様書.md` | 62, 63 | — |
| 65 | **Home導線**: `HomePage.tsx`のアクション行に`ToolLibraryButton`（`/tools`へ遷移する純粋なナビゲーション）を追加し3ボタン化（新規作成・JSONインポートと同格ピル・モバイル折返し可）。**空状態`EmptyState`（homeバリアント）のCTAは新規作成・インポートの2つのまま変更しない**（レシピ作成が主目的のため）。i18n`home.toolLibrary`等 | `src/routes/HomePage.tsx`更新 / `src/components/home/`配下新規または更新 / ja/en/fr/de/it/es/ko.json更新 / テスト | 22 | ✅（3ボタン描画・クリックで`/tools`遷移・EmptyStateのボタン数不変の回帰）＋🖐（375/768/1280折返し＋ヒットテスト） |
| 66 | **Setupツール登録削除**: `RecipeSetupPage.tsx`から`ToolListEditor`を除去し、誘導文言`ToolLibraryHint`（1行・`/tools`および工程エディタへの案内リンク付き）に置換。`components/setup/ToolListEditor.tsx`と同testを**削除**。孤児i18nキー削除（`setup.addTool`/`toolPlaceholder`/`usedInSteps`/`unused`/`inUseNote`の5キー×7ロケール。**`setup.toolsLabel`はnoteMarkdown.tsエクスポータが見出しに使用中のため削除禁止**）。誘導文言i18nキー新設 | `src/routes/RecipeSetupPage.tsx`更新 / `src/components/setup/ToolListEditor.tsx`削除＋同test削除 / ja/en/fr/de/it/es/ko.json更新 / テスト | 23 | ✅（`ToolListEditor`非描画・誘導文言表示＋`/tools`リンクhref・削除5キーの未使用確認・`setup.toolsLabel`存続確認・i18n棚卸しテストpass） |
| 67 | **UpdatesPage新設**: 新規`routes/UpdatesPage.tsx`（HelpPage同型: `BackLink`→hero→エントリ`section`列＝日付mono＋見出し＋本文。i18nは文字列リテラルキーのみ）。`router.tsx`に`/updates`追加（`/help`遡及文書化とあわせ総数表記は10ルート）。`AppFooter.tsx`に4リンク目を追加。i18n`updates.*`/`nav.updates`新設。初期コンテンツ＝初期リリース〜M13の主要機能をユーザー向け文言で日付付きに整理 | `src/routes/UpdatesPage.tsx` / `src/router.tsx`更新 / `src/components/common/AppFooter.tsx`更新 / ja/en/fr/de/it/es/ko.json更新 / テスト | 3, 5, 35 | ✅（`/updates`直URL描画・エントリ表示・フッター4リンク目の遷移）＋🖐（375/768/1280直URLリロード＋ヒットテスト） |

**M14完了条件**: T64〜T67がreview PASS＋出口実機で「Homeアクション行からツールライブラリ遷移・Setupにツール登録UIが無く誘導文言から`/tools`遷移・`/updates`直URL 200＋初期エントリ表示・フッター4リンク・375/768/1280横はみ出し0＋ヒットテスト」
