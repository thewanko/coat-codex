...
- **商標表記**（docs/legal/coat-codex_商標表記.md、2026-07-02ユーザー納品）: T35でTermsPage長文＋AppFooter短文として実装。**連絡先メールアドレスが未確定** — プレースホルダのまま公開禁止（確定まで連絡先段落は省略）。ユーザー判断待ち（専用アドレス or 既存アドレス）。商用要素追加前は専門家レビュー推奨の注記あり
