...
| **ToolsPage**（v2.6新設・§2.8） | `BackLink`／追加行（input＋追加ボタン・Enter確定・重複無視）／一覧（name昇順・各行に`TagChipEditor`＋削除✕） | ツールライブラリ（`userTools`）の一覧・追加・タグ付け・削除・専用エクスポート/インポート。削除は`ConfirmDialog`（description=「登録済みレシピからは削除されません」）経由。0件時`EmptyState`。`AppFooter`に常設導線 |
