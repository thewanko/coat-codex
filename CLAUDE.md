...
- アニメーション付きオーバーレイUIは座標検証前に `el.getAnimations().forEach(a => a.finish())`（非表示タブはCSSアニメーションが currentTime=0 で停止する）。**さらに背景タブはviewportが0×0のままレイアウトされず`elementFromPoint`が全対象でnull（偽陰性）になることがある**＝座標検証の前に`innerWidth > 0`を確認し、0ならpreview_screenshot/preview_resizeで描画を強制してから検証する（スクショは正常描画でもevalの座標系が死んでいる、という乖離が起きる）。**さらに背景タブではfocus()/blur()がactiveElementを変えずfocusout/focusinが発火しない**（.click()は動くため気づきにくい）＝onBlur/onFocus起点のUIが不発でも実バグと即断せず、①`__reactProps`でハンドラ結線とcontrolled valueを確認②`el.dispatchEvent(new FocusEvent('focusout',{bubbles:true}))`等フレームワークが購読する下位イベントを明示発火して結果状態を実測③RTLで同経路の回帰固定を確認、の3点緑なら「検証環境の限界」と記録して合格にする (lessons.md昇格 2026-07-08: 非表示タブのレンダリング停止2回目=viewport相／2026-07-09: 3回目=focus相を追記)
...
- 仕様の正: codex = `docs/coat-codex_技術計画_v2.md`（v2.4）／Scriptorium = `docs/coat-scriptorium_技術計画_v1.md`（v1.7）。ビジュアルの正: `docs/design/coat-codex_デザイン仕様書.md`＋`docs/design/handoff/coat-codex 決定デザイン.dc.html`
