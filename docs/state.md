...
- **SNS共有はパーツ単位**（2026-07-03決定、v2.3）: X/Bluesky共通。**画像=工程ごとの1枚絵（全体画像＋工程写真＋工程情報）を候補生成し、ユーザーが最大4枚選択**（既定=先頭4枚）。投稿テキストにURL非掲載（Xリーチ抑制対策）・`#coat-codex`必須（トリム対象外）。起点=PartCardメニュー、ExportActionBarからX/Bluesky廃止。詳細は計画§3.4冒頭とT37/T39/T40
