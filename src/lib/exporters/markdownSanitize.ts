// lib/exporters/markdownSanitize.ts — Markdownエクスポータ用の最小サニタイズ（M5レビューRound1修正3）
//
...
//      引用>・数字リスト等）で始まる場合のみ、その先頭記号をゼロ幅スペースで無害化する
//      （視認上はほぼ変化なし。バックスラッシュエスケープより人間可読性を保てるため採用）。

const LEADING_MARKDOWN_MARKER_RE = /^(\s*)([#>*+-]|\d+[.)])/;

/** ゼロ幅スペース（U+200B）。行頭のMarkdown構文記号とレンダラの間に挟み、構文発火を防ぐ */
 ...
 *   記号の直後にゼロ幅スペースを挿入して構文としての解釈を無害化する
 */
export function sanitizeMarkdownText(value: string): string {
  ...
  const [full, leadingSpace, marker] = match;
  return (
    leadingSpace + marker + ZERO_WIDTH_SPACE + collapsed.slice(full.length)
  );
}
