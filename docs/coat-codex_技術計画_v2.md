...
| 37 | `sns/imageComposer.ts`: Canvasで投稿用合成画像**最大4枚**生成（**v2.3: 対象パーツの写真つき工程〔`steps[].photoId`非null〕から時系列（工程順）で自動選定。代表写真は使わない**。合計≠100の工程を含む場合は警告表記を継承 — §2.3）、`File[]`（image/png）返却。写真選定ロジックは純関数分離（partIdを引数に） | `src/lib/sns/imageComposer.ts` / テスト | 14 | ✅（選定純関数）＋🖐 |
