...
**改訂履歴**: 2026-07-02 v2.2: デザイン決定稿（[デザイン仕様書](design/coat-codex_デザイン仕様書.md)§8、検討t4/t5合意）の提案A（工程写真1枚紐づけ）・提案B（MIX%再設計）を反映。**要件定義10-3の「パーツ写真複数枚」「%とA:B比率の両保持」からの合意済み変更**（原典は改訂せず本書を正とする）。／2026-07-03 v2.3: **SNS共有をパーツ単位に変更**（レシピ全体は1投稿でシェアしきれないため。ユーザー決定）・投稿テキストに**URLを含めない**（Xのリンク付き投稿リーチ抑制対策）・**`#coat-codex`タグ必須**（§3.4）。プリセット3ブランド確定（§6）。
...
| | `PartCardList`（dnd-kit Sortable）→ `PartCard` | **パーツカードのD&D並び替え**（§3.2の(2)）。サムネ・工程数・混合バッジ表示、タップで10-3へ。**サムネ＝写真がある最後の工程の写真（なければプレースホルダ）＋`STEP n`タグ**（v2.2: デザイン決定稿§8-A反映）。**混合バッジの文字列は `formatMixBadge` の出力をそのまま表示する（単一情報源）**: 合計100時＝**「60% + 40% (3:2)」**形式（約分不能時は比率省略「55% + 45%」）、合計≠100時＝比率省略＋警告併記（v2.2: §2.3の書式が正）。デザインパッケージのバッジ表記もこの形式に従う。**v2.3: カードメニューに「このパーツを共有」（X/Bluesky→ShareDialog起動。§3.4）を追加**。**パーツ0件時は `EmptyState`（partsバリアント: パーツ追加CTA付き）** |
| | `AddPartButton` | パーツ追加→10-3へ |
| | `ExportActionBar` | 印刷／**PDFダウンロード（確定機能として明示ボタンを設置。実現方式のみ§6未決）**／note.com向けMD／**JSONエクスポート・素のMarkdownエクスポート（要件どおり隣接配置）**。**v2.3: X/Blueskyボタンはここから廃止しPartCardメニューへ**（§3.4。`SnsTarget`配列登録制は不変）。JSONエクスポート成功時は `meta` の `recipeExport:<recipeId>` を更新（§3.5） |
...
| **PartEditorPage** (10-3) | `PartEditorHeader` ＋ `StepPhotoStrip` | 通常: `PartNameInput`（**パーツ写真ギャラリーはv2.2で廃止 — 工程写真はStepCard内の`StepPhotoTile`へ。デザイン決定稿§8-A反映**）／**baseモード: 固定見出し「ベース工程（全体）」＋代表写真の読み取り専用サムネ**（全体写真の編集はSetupに誘導）。**モバイルのみヘッダ直下に`StepPhotoStrip`＝写真つき工程のサムネを番号付きで横並び表示し、タップで該当工程（StepCard）へスクロール**（v2.2: §8-A）。工程編集UI以下は両モード完全共通 |
...
| | `StepCard` | `TechniqueSelect`（プリセット or 自由入力）／`PaintSlotList`（max5、A〜E）→ `PaintSlot`＝`BrandSelect`+`ColorSelect`+`SwatchChip`+**%入力**／`MixRatioInput`（**%主体入力＋比率入力補助＋「計 n%」表示。合計≠100はerror皮＋メッセージ＋比率欄「—」disabled — §2.3。v2.2: デザイン決定稿§8-B反映**）／`ToolSelect`／**下段に`StepPhotoTile`（工程写真1枚: 84pxタイル・`STEP n`タグ・空は破線「＋ 写真 1枚」）＋`MemoField`のペア（v2.2: §8-A）**／工程削除 |
| | `AddStepButton` | 工程追加 |
...
> **v2.3改訂（2026-07-03ユーザー決定）: SNS共有は「パーツ単位」**。レシピ全体は1投稿でシェアしきれないため、X・Bluesky共通でパーツを単位とする（印刷／PDF／Markdown／JSONは従来どおりレシピ全体）。**投稿テキストにURLは含めない**（Xのリンク付き投稿リーチ抑制対策。レシピ個別URLが存在しないクライアント完結アプリでもある）。**`#coat-codex`タグをデフォルトテキストに必ず含める**。ベース工程（全体）の共有は対象外（必要性が出たら再検討）。デザイン決定稿のExportActionBarラフにあるX/Blueskyボタンはこの改訂によりPartCard側へ移る。

分岐は**UA判定ではなく機能検出**（`navigator.canShare({ files })`）で行う。結果的にモバイルは主にA系統、デスクトップは主にB系統になる。

**共通前処理（ShareDialogオープン時）**
1. **`PartCard`（Overview）のメニュー「このパーツを共有」**押下 → 対象パーツを文脈に `ShareDialog` を開く（`SnsTarget` 登録制は不変。v2.3: ExportActionBarのX/Blueskyボタンは廃止）。
2. **ダイアログ表示と同時に** `imageComposer` が投稿用合成画像の**候補カード**を生成開始。**v2.3: 対象パーツの写真つき工程（`steps[].photoId`非null）それぞれについて「全体画像（代表写真）＋パーツ画像（当該工程の写真）＋工程情報（工程番号・技法・塗料スウォッチ/混合バッジ）」を1枚絵に合成**した候補カードを工程順に全件生成し、**ユーザーが`ShareImagePreview`で最大4枚を選択**する（既定=時系列の先頭4枚を選択済み。選択変更は生成済み`File`の組み替えのみで再生成しない=transient activation維持）。写真つき工程が0件のパーツはA系統を「テキストのみで共有」・B系統をIntentのみに切替。`File[]`（image/png）として保持完了させる — **後段の `navigator.share()` をユーザークリック直下（transient activation内）で同期的に呼ぶため、クリックハンドラ内で非同期生成しない**。**生成中はダイアログの `ShareImagePreview` にプレースホルダ＋進行表示（「画像を生成中…」）を出し、A系統の主ボタンとB系統の「合成画像をダウンロード」ボタンは生成完了までdisabledにする**（活性条件＝`File[]` 保持完了）。**生成失敗時**はエラートーストを表示し、A系統では「テキストのみで共有」に主ボタンを差し替え（`navigator.share({ text })`）、B系統ではDLボタンを非活性のままIntentボタンのみ有効とする。
3. 投稿テキストを自動生成（**v2.3: レシピタイトル＋パーツ名＋工程サマリ＋`#coat-codex`。URLは含めない**。編集可）。ターゲット別カウンタを表示: X＝重み付き280字（CJK=2）、**Bluesky＝`Intl.Segmenter` による300 grapheme上限**。超過時は警告＋自動トリムボタン（`#coat-codex`はトリム対象外＝末尾維持）。
...
> ③ **palette孤児エントリの方針**: custom色の内容変更は新エントリ追加+旧孤児化になる（update-in-placeなし）。M4で「未参照palette色の削除UI（PaletteEditorの使用数0削除で兼ねる）」を確認し、不足ならGCまたは同一スロット再確定時の更新方式を検討する
...
| 32 | 素のMarkdown `exporters/markdown.ts`＋note.com向け `exporters/noteMarkdown.ts`＋**スナップショットテスト（指摘8）**: 代表フィクスチャ（**混色〔合計100・比率併記／約分不能・比率省略／合計≠100・警告併記〕**/単色/塗料0件/**工程写真あり・なし（v2.2）**/ベース工程/複数パーツ/自由入力技法を網羅）で`toMatchSnapshot` | `src/lib/exporters/markdown.ts`・`noteMarkdown.ts` / `tests/exporters.snapshot.test.ts` / `tests/fixtures/recipe.ts` | 7, 8, 9 | ✅（スナップショット） |
...
| 37 | `sns/imageComposer.ts`: **v2.3: 工程ごとの合成カード生成** — 対象パーツの写真つき工程〔`steps[].photoId`非null〕それぞれを「全体画像（代表写真）＋工程写真＋工程情報（番号・技法・スウォッチ/混合バッジ。合計≠100は警告表記を継承 — §2.3）」の1枚絵に合成し全候補を`File[]`（image/png）で返却。**最大4枚の選定はユーザー選択（T39）**。候補列挙・カードレイアウト計算は純関数分離（partIdを引数に） | `src/lib/sns/imageComposer.ts` / テスト | 14 | ✅（候補列挙・レイアウト純関数）＋🖐 |
...
| 39 | **`ShareDialog`＝Web Share 2系統（指摘12）**: 分岐は`navigator.canShare?.({ files })`の**機能検出**。ダイアログopen時に合成画像を事前生成しFile[]保持（**クリックハンドラ内で非同期生成しない**=transient activation維持）。**A系統**: 「共有シートで投稿」→ハンドラ内で同期的に`navigator.share({ text, files })`、`AbortError`無視・`NotAllowedError`等はB系統UIへフォールバック、副導線リンク常設。**B系統**: 手順ガイド（①DL②投稿画面③手動添付）＋連番PNG一括DL＋Intent新規タブ（**Intent URLは画像添付不可の案内を常時表示**）。`ShareImagePreview`（**v2.3: 選択式** — 工程順の候補カード一覧からチェックで最大4枚選択・既定=先頭4枚・選択数表示・5枚目はdisabled）／`ShareTextEditor`（ターゲット別カウンタ）。**v2.3: partIdを受け取るパーツ単位ダイアログ。テキスト既定=タイトル＋パーツ名＋工程サマリ＋`#coat-codex`・URLなし・タグはトリム対象外。写真つき工程0件パーツはテキストのみ共有に切替（§3.4）** | `src/components/overview/ShareDialog.tsx`・`ShareImagePreview.tsx`・`ShareTextEditor.tsx` | 28, 37, 38 | ✅（canShareモックの分岐unit）＋🖐（iOS Safari/Android Chrome/デスクトップの3環境） |
| 40 | 共有導線の最終結線（v2.3）: `PartCard`メニュー「このパーツを共有」→ShareDialog（→T39、SnsTarget配列駆動）／`ExportActionBar`は印刷・PDF（→T36）・note MD・JSON・素MD（T33済みの確認）のみ=X/Blueskyボタンなし | `src/components/overview/PartCard.tsx`・`ExportActionBar.tsx`更新 | 33, 36, 39 | 🖐 |
