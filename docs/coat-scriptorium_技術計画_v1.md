# Coat Scriptorium（coat書庫）技術計画 v1.6
...
- `app.ts` — Hono ルート定義。Bindings 型: `{ DB: D1Database; BUCKET: R2Bucket; AI?; TURNSTILE_SECRET; IP_HASH_SECRET; MAIL_API_KEY?; NOTIFY_EMAIL_TO?; NOTIFY_EMAIL_FROM? }`（v1.5: 実装に合わせ確定。AI は workers-types 版都合で構造的型 AiRunner）
...
/r/:id           → RecipeDetailPage（cover・palette・tools・baseSteps・parts・
                    「coat-codexにインポート」・通報ボタン・削除ボタン（ST-35 本人削除）・ハンドル表示）
...
### S6: モデレーション＋本人削除UI（完了条件: 通報→自動非公開＋通知・承認制/遮断フラグ即時反映・本人削除がUIから可能 = **パブリックローンチのゲート**）
...
| ST-35 | 本人削除 UI（詳細ページ: 削除 PW 入力ダイアログ→`DELETE /api/recipes/:id`→完了表示。403/429/404 のエラー別表示・「反映まで最大 5 分」注記〔§4.5 TTL〕。Turnstile 不要=DELETE は rate limit del:5/日/IP/レシピで防御〔§3.4〕） | `DeleteRecipeDialog.tsx`・RecipeDetailPage 更新 | ST-15, ST-19 | unit test（成功・403・429 分岐）＋🖐 実削除→TTL 失効後に一覧/詳細から消える |
...
### S8: リリース仕上げ＋公開前セキュリティ硬化

公開前セキュリティチェックリスト（2026-07-08 ユーザー提供・11節）を実装と突き合わせた結果、**入力検証・SQLi 対策・レート制限・PBKDF2 削除 PW・Turnstile fail-closed・XSS sink・画像マジックバイト・EXIF 除去・シークレット衛生は実装済み＆テスト済み**（詳細は付録 A の突き合わせ表）。残るギャップ = セキュリティ応答ヘッダー／プライバシーポリシー／flagged 画像の R2 残存／pages.dev noindex／履歴シークレットスキャン。**新規マイルストーンは立てず S8 に統合**（ユーザー裁定 2026-07-08）。ST-36〜39 はコード硬化、ST-33/34 に i18n と攻撃者視点 E2E を追記。ST-36〜39 は S7 に依存せず独立実装可（ローンチ前倒しが必要なら S7 前でも回せる）。

| # | タスク | 成果物 | 依存 | 完了条件 |
|---|---|---|---|---|
| ST-33 | i18n 棚卸し（en/ja 網羅機械チェック）＋エラーメッセージ最終。**ST-37 の `privacy.*` キーを含める** | locales＋テスト | ST-11〜ST-32, ST-37 | `npm test` 緑 |
| ST-34 | 通し QA＋**攻撃者視点 E2E**（付録 B）: 投稿制限 429・PW 削除・通報→非公開→TTL 反映・承認制切替・circuit・OGP・CORS・深リンクリロード・重複インポート・Access 保護・R2 削除確認／偽 MIME（.jpg を名乗る HTML・SVG）拒否・`<script>`/`onerror` の一覧/詳細/codex インポート後 3 画面非発火・削除 PW 総当たり→429・Turnstile なし/使い回し拒否・通報連打→閾値 flagged・削除済み画像 URL 404・**gitleaks 履歴スキャン** | QA チェックリスト（docs 追記） | 全タスク | 🖐（iPhone 含む）＋各項目の証跡を transcript 表示 |
| ST-36 | **セキュリティ応答ヘッダー**（CSP・Referrer-Policy・X-Frame-Options・Permissions-Policy・nosniff）。scriptorium は advanced mode（`dist/_worker.js`）のため `_headers` 不可→`worker.ts` の `withSecurityHeaders()` でドキュメント応答に注入。codex は純静的のため `public/_headers`（`connect-src` に `https://scriptorium.coat-codex.com` を追加）。両アプリ CSP: `script-src 'self' https://challenges.cloudflare.com`／`style-src 'self' 'unsafe-inline' https://fonts.googleapis.com`／`font-src` gstatic／`img-src 'self' data: blob:`／`frame-src` challenges／`frame-ancestors 'none'`（インラインscriptゼロ＝`script`に`'unsafe-inline'`不要） | `apps/scriptorium/src/server/worker.ts`・`apps/codex/public/_headers` | ST-14 | build/lint/test 緑＋🖐 両アプリ主要フロー全通しでコンソール CSP 違反ゼロ＋本番/プレビュー URL の応答ヘッダー実在（curl 正例アサーション） |
| ST-37 | **プライバシーポリシーページ**（チェックリスト§10 公開必須）: IP ハッシュ一時保持・削除 PW の扱い・Cloudflare インフラ利用・トラッキング無し・cover 保存・contact 窓口を明記。Terms/ContentPolicy 同型 | `apps/scriptorium/src/routes/PrivacyPage.tsx`・route `/privacy`・footer・`privacy.*` i18n（en/ja 実訳＋他5 en placeholder） | ST-17 | build/lint/test 緑＋RTL レンダーテスト＋🖐 /privacy 描画・フッター導線（**最終文面はユーザー確認**） |
| ST-38 | **flagged 化時の R2 画像削除**: 通報閾値到達で published→flagged に実遷移した時のみ（changes>0）cover/thumb を R2 best-effort 削除（deleteRecipe 同型）。復帰トレードオフは §8-11 | `apps/scriptorium/src/server/routes/report.ts` | ST-26 | build/lint/test 緑＋回帰テスト（遷移時削除・未満は非削除・R2 失敗でも通報成功）＋🖐 threshold=1→通報→flagged→/img 直 URL 404 |
| ST-39 | **robots.txt／*.pages.dev noindex**: 両アプリ `public/robots.txt`（本番 index 許可）。scriptorium は `worker.ts` で hostname が `.pages.dev` 終端なら `X-Robots-Tag: noindex`。codex 分の pages.dev noindex は Transform Rule（§9 ユーザーアクション） | `apps/scriptorium/public/robots.txt`・`apps/codex/public/robots.txt`・`worker.ts`（ST-36 と同一ファイル） | ST-36 | build/lint/test 緑＋🖐 /robots.txt 200・pages.dev 応答に `X-Robots-Tag: noindex`（scriptorium） |
...
4. **メール通知プロバイダ = Resend で確定**（2026-07-08 ユーザー裁定・ST-27 実装済み）: MailChannels の Workers 無料連携は終了済み。Cloudflare Email Routing は受信専用で送信不可のため不採用。**Cloudflare Email Sending（β）も検討したが不採用**——docs には「検証済み宛先への送信は全プラン無料」とあるものの、dashboard 実挙動はオンボード自体が Workers Paid 専用ゲート（ユーザー実測 2026-07-08）で、かつ `send_email` バインディングは Pages 非対応。採用 = **Resend REST API**（無料 100 通/日・`moderation/notifier.ts` が `MAIL_API_KEY`/`NOTIFY_EMAIL_TO`/`NOTIFY_EMAIL_FROM`〔既定 onboarding@resend.dev〕で送信・transport 注入設計のため将来の差し替えは 1 関数）。ドメイン未認証の間は宛先 = Resend アカウント本人のメールのみ。初週は report_threshold=1 で保守的に運用開始できる（settings ノブ）
...
11. **flagged 画像削除と S7 復帰のトレードオフ**（ST-38）: 通報閾値到達で flagged 化する際に R2 の cover/thumb を削除する。flagged は S7 admin で published へ復帰可能だが、**画像を消すと復帰してもレシピ本文（テキスト）は戻るが cover は空になる**。通報の主眼が NSFW/著作権等の不適切コンテンツ即時遮断であることから、画像の即時除去を優先する既定を採用（ユーザー裁定 2026-07-08）。代替（/img が所有レシピの status を都度 D1 参照して非 published を 404）は immutable キャッシュ／R2 直配信モデルを壊すため不採用
12. **CSP と Pages 配信形態の差**（ST-36）: scriptorium は `dist/_worker.js`（advanced mode）のため Pages の `_headers` ファイルは無視される→セキュリティヘッダーは `worker.ts` で応答をラップして注入する。codex は純静的 SPA のため `public/_headers` が有効（CLAUDE.md の作成禁止対象は `_redirects`/`404.html` のみで `_headers` は可）。両アプリともインラインスクリプトゼロのため `script-src` に `'unsafe-inline'` は付けない。`style-src 'unsafe-inline'` のみ React の `style={{}}` 属性と Google Fonts CSS のため必要。CSP は機能を静かに壊すため、ST-36 完了条件に「主要フロー全通しでコンソール違反ゼロ」を必須化する
...
| ST-27 前 | メール通知プロバイダの裁定・契約（候補 Resend）・API キー発行 → **完了（2026-07-08: Resend 確定・`MAIL_API_KEY` Secret 設定済み。残 = `NOTIFY_EMAIL_TO`〔平文変数・Resend 登録メール〕の設定）** |
| ST-30 後 | scriptorium Pages のビルド環境変数に `VITE_TURNSTILE_SITEKEY`（codex と同一値・平文でよい。Turnstile ホスト許可は coat-codex.com がサブドメイン包含のため追加不要） |
| ST-29 後（任意） | scriptorium Pages の Bindings に Workers AI（`AI`）を追加（nsfw_screening を 'on' にする場合のみ。既定 off のため必須ではない） |
| ST-31 | Cloudflare Access アプリケーション作成（/admin*・/api/admin* ポリシー・管理者メール許可） → **完了（2026-07-09: One-time PIN・本番302/ログイン実証済み）** |
| 各シークレット | TURNSTILE_SECRET / IP_HASH_SECRET / MAIL_API_KEY を Pages の環境変数（Secret）に設定 |
| S8（セキュリティ硬化） | **HTTPS 強制＋HSTS** 有効化（Always Use HTTPS・両ドメイン） → **完了（2026-07-09 ユーザー実施）** |
| S8 | **WAF 無料マネージドルール・Bot Fight Mode** の有効化を検討 |
| S8（ST-39 補完） | ~~Transform Rule~~ → **codex `public/_headers` のホスト指定ルールで対応済み（v1.7 是正・2026-07-09）**: Transform Rule は自ゾーンのトラフィックにのみ適用され Cloudflare 所有の `*.pages.dev` には効かないため実現不可。Cloudflare Pages 公式の `_headers` ホスト指定プレースホルダー（`https://:project.pages.dev/*` ＋ `https://:version.:project.pages.dev/*` → `X-Robots-Tag: noindex`）で本番＋プレビューデプロイの pages.dev のみ noindex（カスタムドメイン非適用・ダッシュボード作業不要）。scriptorium は worker.ts 注入（ST-39）で対応済み＝2026-07-09 本番 curl 確認（pages.dev=noindex あり・カスタムドメイン=なし・robots.txt 200） |
| S8 | **使用量アラート**: Workers リクエスト・D1 書込・R2 容量のアラート設定。上限到達時の挙動（新規投稿停止＝閲覧継続）を確認 |
| S8 | **D1 バックアップ**: Time Travel 確認＋定期エクスポート手順の文書化（個人開発の最頻出＝自ミスでの D1 消失に備える）。R2 含む復旧手順を docs 化 |
| S8 | **バインディング最小権限**: D1/R2 バインディングが不要な DB/バケットへの権限を持たないことを確認 |
| S8（任意・推奨） | **gitleaks を CI/git hook に統合**（ST-34 の一度きり履歴スキャンとは別に、継続的なシークレット混入検知） |
...
## 付録 A: 公開前セキュリティチェックリスト突き合わせ（実装済み・対応不要）

2026-07-08 に全11節を実コード照合。以下は既に実装済み＆テスト済みで**今回対応不要**（ギャップのみ S8 ST-36〜39／ST-33/34 で対応）。

| チェック項目 | 実装箇所 |
|---|---|
| 入力検証（zod strict・長さ上限・参照整合・未知フィールド拒否） | `packages/recipe-core/src/schema/published.ts`（`publishedRecipeStrictSchema`。title 120/name 80/note・memo 2000/parts 50/steps 200/palette 100/64KB） |
| ボディ全体サイズ上限 | `server/routes/postRecipe.ts`（Content-Length 640KB→パート精査。cover 450KB/thumb 80KB） |
| SQLi（全クエリ prepare/bind・連結ゼロ・LIKE なし） | postRecipe/deleteRecipe/report/feed/settings/recipePage |
| エラー内部情報秘匿 | 全経路 try/catch＋汎用メッセージ |
| Turnstile fail-closed（投稿・通報必須・remoteip 送信） | `server/guards/turnstile.ts` |
| レート制限＋自動 circuit（post5/del5/report10・global hourly30・HMAC IP・lazy prune） | `server/guards/rateLimit.ts`・`circuitBreaker.ts` |
| 削除 PW（PBKDF2-SHA256 10万・定数時間比較・平文非出力） | `server/auth/password.ts` |
| 通報閾値→flagged＋一覧/詳細除外 | `report.ts`・`feed.ts`（status='published' のみ） |
| 削除時 R2 削除・/img 404 | `deleteRecipe.ts`・`app.ts`（flagged は ST-38 で追加） |
| XSS sink ゼロ（dangerouslySetInnerHTML/innerHTML/Markdown なし） | apps/・packages/ 全体 |
| OGP HTML エスケープ | `server/ogp.ts`（`escapeHtmlAttribute`＋`encodeURIComponent`） |
| 禁止パターン（javascript:・`<`・URL 拒否）・coverUrl 検証・importLink ハードコード | `published.ts` `FORBIDDEN_TEXT_PATTERNS`・`importFromScriptorium.ts`・`importLink.ts` |
| インポート経路のスキーマ検証（他人 JSON） | `exchange/importPipeline.ts`（3段）・`importFromScriptorium.ts`（envelope zod） |
| 画像マジックバイト（JPEG/WebP・PNG/SVG 拒否）・寸法/サイズ上限・パストラバーサル | `server/images/imageHeader.ts`・`app.ts` `isAllowedImageKey` |
| EXIF 除去（canvas 再エンコード）・R2 put 唯一経路 | `apps/codex/src/lib/coverComposer.ts`・put は `postRecipe.ts` のみ |
| 削除 PW のクライアント非保存 | `publishToScriptorium.ts`（meta は id/url/postedAt のみ） |
| シークレット衛生（.dev.vars gitignore・wrangler.toml 平文なし・npm audit 0） | ルート `.gitignore`・両 `wrangler.toml` |

## 付録 B: 攻撃者視点 E2E（ST-34 で実施）

- 巨大 JSON（数 MB）／巨大画像／**偽 MIME（.jpg を名乗る HTML・SVG）**→全て拒否（400/413）
- テキスト欄に `<script>alert(1)</script>`／`"><img src=x onerror=...>` を投稿試行→禁止パターンで拒否、通過分は**一覧・詳細・codex インポート後**の 3 画面で非発火
- 削除 PW 総当たりスクリプト→レート制限（del 5/日/レシピ）で阻止
- Turnstile トークンなし／使い回し→拒否（fail-closed）
- 通報連打→レート制限＋閾値 flagged＋（S7 後）管理者復帰
- 削除済み投稿の画像 URL・API が 404
- （S7 実装後）未ログインで /admin・/api/admin 直叩き→拒否（Access 302＋Worker JWT 検証）
- **gitleaks 履歴スキャン**（`gitleaks git` or `detect`）で全コミット履歴のシークレット混入確認。検出時はキーをローテーション

---

## 改訂履歴

- v1.7（2026-07-09）: **§9「codex 分の pages.dev noindex = Transform Rule」を実現不可と確定し `_headers` ホスト指定方式へ是正**。Transform Rule は自ゾーン（coat-codex.com）のトラフィックにのみ適用され、Cloudflare 所有ゾーンである `*.pages.dev` への応答には効かない（ダッシュボード作業前の 1 次確認で捕捉）。Cloudflare Pages 公式ドキュメントの `_headers` ホスト指定プレースホルダー（`https://:project.pages.dev/*`・`https://:version.:project.pages.dev/*`）で `X-Robots-Tag: noindex` を付与する方式を本採用し `apps/codex/public/_headers` へ実装（ユーザーアクションからコード管理へ移行・プレビューデプロイも網羅）。scriptorium 分（ST-39 worker.ts 注入）は本番 curl で noindex 実在・カスタムドメイン非付与・robots.txt 200 を確認済み
- v1.6（2026-07-08）: **公開前セキュリティチェックリスト（ユーザー提供・11節）を実装と突き合わせ、ギャップを S8 に統合**（ユーザー裁定 2026-07-08: 新規マイルストーンは立てず S8 リリース仕上げに統合・コード硬化4点すべて今回スコープ）。3体の Explore で全項目を実コード照合した結果、入力検証・SQLi・レート制限・PBKDF2・Turnstile・XSS sink・画像マジックバイト・EXIF 除去・シークレット衛生は実装済み＆テスト済みと確認（付録 A）。残ギャップにコード硬化タスク ST-36（セキュリティ応答ヘッダー・CSP。scriptorium は advanced mode で `_headers` 不可→`worker.ts` 注入、codex は `public/_headers`＝§8-12）・ST-37（プライバシーポリシーページ）・ST-38（flagged 化時の R2 画像削除＝§8-11 の復帰トレードオフ明記）・ST-39（robots.txt／pages.dev noindex）を新設し、ST-33（i18n に `privacy.*` 追加）・ST-34（攻撃者視点 E2E＋gitleaks 履歴スキャン＝付録 B）を拡張。§9 に HSTS・WAF・使用量アラート・D1 バックアップ・pages.dev Transform Rule 等のユーザーアクションを追記
- v1.4（2026-07-08）: **本人削除 UI タスク ST-35 を S6 に追加**（ユーザー裁定 2026-07-08）。DELETE API（ST-19）は S4 で実装・デプロイ済みだが UI を作るタスクがどの ST にも未割り当てで、マージ済み PostGuide の「レシピページからいつでも削除でき」の文言が実現手段のない約束になっていた欠落の是正。§5.1 詳細ページに削除ボタンを追記し、S6 完了条件（パブリックローンチのゲート）へ本人削除 UI を含めた
- v1.5（2026-07-08）: **S6 実装完了に伴う §8-4 決着と §9 更新**。メール通知プロバイダ = Resend 確定（Cloudflare Email Sending β は dashboard 実挙動が Workers Paid 専用ゲート＋send_email バインディング Pages 非対応のため不採用）。env 名を実装に合わせ確定（`MAIL_API_KEY`/`NOTIFY_EMAIL_TO`/`NOTIFY_EMAIL_FROM`。旧記述 ADMIN_EMAIL は NOTIFY_EMAIL_TO へ改称）。§9 に scriptorium 用 `VITE_TURNSTILE_SITEKEY`・任意 AI バインディングの行を追加
