...
| S8（ST-39 補完） | **codex 分の *.pages.dev noindex**: Transform Rule（Modify Response Header）で hostname contains `.pages.dev` → Set `X-Robots-Tag: noindex`（1 ルールで両プロジェクト網羅可・scriptorium はコードでも二重防御） |
