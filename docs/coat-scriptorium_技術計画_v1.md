...
- v1.3（2026-07-07）: 本番デプロイ実証を反映。`functions/` ディレクトリ方式は Root=`/` 構成の本番 Git ビルドで検出されない（ローカル pages dev では検出できない差）ため廃止し、**`dist/_worker.js`（advanced mode）を本採用**（§1.1・§4.3。src/server/worker.ts＋recipePage.ts・esbuild バンドル）
