...
- レビュー指摘の修正バッチ後は必ず同一レビュアーでRound N+1判定を取り、「修正同士の複合作用・修正が導入した新規問題」を明示観点に含める。修正がUI状態管理（key・effect・確定タイミング）やオーバーレイUIのDOM位置（リフトアップ・portal化・レンダー先変更）に触れる場合は、「修正が無効化した暗黙の前提」（z-index序列・スタッキング文脈・イベント伝播経路）を観点に明示し、出口実機検証のヒットテストで全インタラクティブ要素を再確認する (lessons.md昇格 2026-07-03)
