# Coat Scriptorium（coat書庫）技術計画 v1
...
index.html の取得手段は 2 案あり **ST-16 の curl 検証で確定**する: ①`context.env.ASSETS.fetch()`（Pages の既定バインディング。利用不可の環境報告もあるため要実機確認）②`context.next()` が返す Response（同パスへのアセットパススルー。SPA フォールバックで index.html が返る）を HTMLRewriter に通す。どちらでも Hono 側ハンドラの構造は変わらない。
...
| ST-12 | Pages プロジェクト `coat-scriptorium` 新規作成（Git 連携・Root directory=apps/scriptorium）＋DNS: scriptorium.coat-codex.com レコード＋カスタムドメイン登録 |
