// components/overview/ExportActionBar.test.tsx — JSON・素MD・note MD・印刷/PDF/X/Bluesky結線
// （技術計画v2.3 §3.3 ExportActionBar行・T28・T33・T40）
//
...
// 印刷/PDFは/recipe/:id/printへのnavigateをreact-routerのMemoryRouter+Routesで確認する
// （RecipeOverviewPage.test.tsxの慣行に従う）。X/BlueskyはShareDialogがwholeコンテキスト・
...
describe("ExportActionBar — PC幅（従来のピル群）", () => {
  ...
  test("recipeがある場合、印刷・PDF・X・Blueskyは有効化される（T40結線）", () => {
  ...
  test("PDFボタン押下で/recipe/:id/printへnavigateする", () => {
    renderBar(makeRecipe());
...
describe("ExportActionBar — mobile幅（出力・共有ボタン→ボトムシート）", () => {
  ...
  test("recipeがある場合、シート内の印刷・PDF・X・Blueskyは有効化される（T40結線）", () => {
    renderBar(makeRecipe());
    fireEvent.click(screen.getByRole("button", { name: "出力・共有" }));
