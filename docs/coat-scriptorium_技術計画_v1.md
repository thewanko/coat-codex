# Coat Scriptorium（coat書庫）技術計画 v1.2
...
2. マージ直後に coat-codex プロジェクトの dashboard 設定を変更（ユーザーアクション §9）:
   ...
   - Build watch paths: `apps/codex/**`・`packages/**`（scriptorium 変更での codex 再ビルド抑止）
...
> **実証結果（2026-07-07 S3出口・v1.2改訂）**: Pages Functions のアダプタは**ルート直下の catch-all（`functions/[[path]].ts`）にしてはならない**。catch-all は静的アセット配信と SPA フォールバックを遮蔽し、`/`・`/terms` 等の全 SPA ルートが 404 になる（wrangler pages dev で実証）。本採用構成は**パススコープマウント**: `functions/api/[[path]].ts`・`functions/img/[[path]].ts`（いずれも `handle(app)` のみの薄アダプタ）＋`functions/r/[id].ts`。Hono アプリ本体（app.ts）は無改変のまま＝§4.1 の移行保険は維持。Functions 起動は /api・/img・/r/:id のみに限定され無料枠消費も減る
